...
def process_pairs(ref_image, ref_mask, tar_image, tar_mask, shape_control=False):
    ...
    masked_ref_image_compose, ref_mask_compose = (
        masked_ref_img_for_collage,
        ref_mask_for_collage * 255,
    ...
    tar_box_yyxx_crop = expand_bbox(
        tar_image, tar_box_yyxx, ratio=[1.5, 3]
    )  # 1.5, 3   #1.2 1.6
    ...
    item = dict(
        ...
        tar_mpi_mask=tar_mask_mpi_cropped,
...
if __name__ == "__main__":
    ...
    for image_name in os.listdir(
        test_bg_dir.replace("/new_samples_1/", "/new_samples_1_mask/")
    ):
    ...
    ob_id_ = (
        -25
    )  # -29 bluw vase, 34 statue, 27 lamp, chair 19 -41 back truck, -36 school bus f2, 40 toy1
    do_null_text_again = False
    ref_img_list = ref_img_list[ob_id_ : ob_id_ + 1]
    ref_img_path = os.path.join(
        ref_image_path, ref_img_list[np.random.randint(0, len(ref_img_list))]
    )
    ...
    for image_name in image_names:
        ...
        tar_mask_path = tar_image_path.replace(
            "/new_samples_1/", "/new_samples_1_mask/"
        )
        tar_mask_name = tar_mask_path.split("/")[-1].split(".")[0]
        tar_mask_path = os.path.join(
            test_bg_dir.replace("/new_samples_1", "/new_samples_1_mask"),
            tar_mask_name + "_mask_0.jpg",
        )
        ...
        cv2.imwrite(
            "{}/mpi_foreground_alpha.png".format(current_save_sir),
            np.array(gt_image_cropped) * mpi_foreground_alpha[1][:, :, None],
        )
        cv2.imwrite(
            "{}/mpi_background_alpha.png".format(current_save_sir),
            np.array(gt_image_cropped) * mpi_foreground_alpha[0][:, :, None],
        )
        ...
        with torch.no_grad():
            ...
            cv2.imwrite(
                "./{}/recon_fg.jpg".format(current_save_sir),
        ...
        gen_single = os.path.join(
            current_save_sir, image_name.split(".")[0] + f"_obj_place_result.png"
        )
        ...
        gen_image = inference_single_image(
            ...
            guidance_scale=5.0,
            curr_save_dir=current_save_sir,
