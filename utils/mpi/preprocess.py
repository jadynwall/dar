...
def get_ddim_inverted_latents(nt_pipeline, image, prompt, num_inference_steps=50):
    ...
    latents = nt_pipeline.ddim_inversion_loop(
        latent, context, num_inference_steps=num_inference_steps
    )
    return latents


def get_null_text_latents(
    nt_pipeline,
    image,
    prompt,
    num_inference_steps=50,
    num_of_optimization_steps=10,
    early_stop_epsilon=1e-5,
):
    ddim_latents = get_ddim_inverted_latents(
        nt_pipeline, image, prompt, num_inference_steps
    )
    context = nt_pipeline.get_context(prompt)
    null_text_emb = nt_pipeline.null_optimization(
        ddim_latents, context, num_of_optimization_steps, early_stop_epsilon
    )
    null_text_emb = torch.stack(null_text_emb, 0)
    null_text_reconstruction, null_text_latents = nt_pipeline(
        prompt, null_text_emb, ddim_latents[-1]
    )
...
def get_bins_for_depth(sam_mask, depth_map, mask_region):
    ...
    for j in range(num_bins):
        ...
        object_mask = cv2.erode(
            object_mask.astype(np.uint8), np.ones((5, 5), np.uint8), iterations=2
        )
        ...
        bins[j] = {"pc": np.where(object_depth != 0), "color": color}
    return bins
...
def get_correct_dimensions(
    ...
    is_coord_x=False,
    num_of_bins=40,
    is_realtive_depth=True,
    debug=False,
):
    if is_realtive_depth:
        depth_bin = [
            depth_close - i * (depth_close - depth_far) / num_of_bins
            for i in range(num_of_bins)
        ]
    else:
        depth_bin = [
            depth_close + i * (depth_far - depth_close) / num_of_bins
            for i in range(num_of_bins)
        ][::-1]
...
def plot_depth_bins(
    depth, sam_mask, mask_image, input_img_name, save_dir, is_crop=False
):
    import matplotlib.pyplot as plt

    input_img_name = input_img_name.split("/")[-1].split(".")[0]
    ...
    for i, bin in enumerate(bins):
        if len(bin) == 0:
            continue
        ax.scatter(bin["pc"][1], bin["pc"][0], -1 * depth[bin["pc"]], c=bin["color"])
    ...
    if is_crop:
        ...
        plt.savefig("{}/{}_depth_bins.png".format(save_dir, input_img_name))
    plt.close()
    # plot front view of depth bins
    fig = plt.figure()
    ...
    if is_crop:
        ...
        plt.savefig("{}/{}_depth_bins_front.png".format(save_dir, input_img_name))
...
def draw_depth_lines(image, y_coor_list, save_dir, input_img_name, chosen_bin=None):
    ...
    cv2.imwrite(
        "{}/{}_depth_lines_debug.jpg".format(save_dir, input_img_name),
        image[:, :, ::-1],
    )
...
if __name__ == "__main__":
    ...
    ntp = NullTextPipeline.from_pretrained(
    ...
    image = (
        Image.open(
            "/mnt/data/rishubh/sachi/AnyDoorV2/examples/Gradio/new_sample/c9.jpg"
        )
        .convert("RGB")
        .resize((512, 512))
    )
