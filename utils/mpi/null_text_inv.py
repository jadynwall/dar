...
class NullTextPipeline(StableDiffusionPipeline):
    ...
    def get_noise_pred(self, latents, t, context=None, is_forward=True):
    ...
    def null_optimization(self, latents, context, num_inner_steps, epsilon):
        ...
        for i in range(self.num_inference_steps):
            ...
            with torch.no_grad():
                ...
                latent_cur = self.get_noise_pred(latent_cur, t, context=context, is_forward=False)
        bar.close()
        return uncond_embeddings_list

    @torch.no_grad()
    def ddim_inversion_loop(self, latent, context, num_inference_steps=50):
        self.num_inference_steps = num_inference_steps
        self.scheduler.set_timesteps(self.num_inference_steps)
        _, cond_embeddings = context.chunk(2)
        all_latent = [latent]
        latent = latent.clone().detach()
        # print("slef.scheduler.timesteps", self.scheduler.timesteps)
        with torch.no_grad():
    ...
    # @torch.no_grad()
    # def ddim_inversion_loop(self, latent, context, num_inference_steps=50, guidance_scale=7.5):
    #     self.num_inference_steps = num_inference_steps
    #     self.scheduler.set_timesteps(self.num_inference_steps)
    #     _, cond_embeddings = context.chunk(2)
    ...
    #         for i in range(0, self.num_inference_steps):
    #             if guidance_scale > 1.:
    #                 model_inputs = torch.cat([latent] * 2)
    #             else:
    #                 model_inputs = latent

    #             t = self.scheduler.timesteps[len(self.scheduler.timesteps) - i - 1]
    #             noise_pred = self.unet(model_inputs, t, encoder_hidden_states=cond_embeddings)["sample"]
    #             if guidance_scale > 1.:
    ...
    def __call__(
        ...
        null_text_latents = [inverted_latent]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                null_text_latents.append(latents)
                progress_bar.update()
        ...
        return StableDiffusionPipelineOutput(images=image, nsfw_content_detected=False), null_text_latents
    ...
    def text2image_ldm_stable(
        ...
        for i, t in enumerate(tqdm(self.scheduler.timesteps[-start_time:])):
            ...
            latents = self.get_noise_pred(latents, t, context=context, is_forward=False)
        ...
        if return_type == 'image':
            ...
            return StableDiffusionPipelineOutput(images=image, nsfw_content_detected=False), latents
        else:
            image = latents
        return image, latent