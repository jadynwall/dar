...
torch.hub.help(
    "intel-isl/MiDaS", "DPT_BEiT_L_384", force_reload=False
...
bg_name = "bg07.png"
bg_img = (
    Image.open(f"./examples/Gradio/scene_comp_data/{bg_name}")
    .convert("RGB")
    .resize((512, 512))
)
...
fg_img = (
    Image.open("./examples/Gradio/scene_comp_data/ab26.jpg")
    .convert("RGB")
    .resize((512, 512))
)
fg_mask = (
    Image.open("./examples/Gradio/scene_comp_data/ab26_mask.png")
    .convert("RGB")
    .resize((512, 512))
)
...
cv2.imwrite(
    "./{}/{}_depth_control.jpg".format(save_dir, input_img_name), depth_control_col
)
depth_control = (
...
cv2.imwrite(
    "./{}/{}_image_mpi.jpg".format(save_dir, input_img_name),
    cv2.cvtColor(image_mpi, cv2.COLOR_RGB2BGR),
)

depth_back = torch.tensor(depth_back).unsqueeze(0).unsqueeze(0).to("cuda")
ten_img2 = (
    torch.tensor(np.array(bg_img)).permute(2, 0, 1).unsqueeze(0).to("cuda") / 255.0
)

if os.path.exists(f"{null_text_emb_path}/{bg_name}_{inv_prompt_bg}_null_text.pt"):
    null_text_emb = torch.load(
        f"{null_text_emb_path}/{bg_name}_{inv_prompt_bg}_null_text.pt"
    ).to("cuda")
    init_noise = torch.load(
        f"{null_text_emb_path}/{bg_name}_{inv_prompt_bg}_init_noise.pt"
    ).to("cuda")
else:
    null_text_emb, init_noise = diff_handles.invert_input_image(
        ten_img2, depth_back, inv_prompt_bg
    )
...
null_text_emb_bg, init_noise_bg, activations_back, latent_image = (
    diff_handles.generate_input_image(
        depth=depth_back,
        prompt=inv_prompt_bg,
...
with torch.no_grad():
    ...
    cv2.imwrite(
        "./{}/{}_recon_bg.jpg".format(save_dir, input_img_name),
        cv2.cvtColor(latent_image, cv2.COLOR_RGB2BGR),
    )

depth_fore = torch.tensor(depth_fore).unsqueeze(0).unsqueeze(0).to("cuda")
ten_img3 = (
    torch.tensor(np.array(fg_img)).permute(2, 0, 1).unsqueeze(0).to("cuda") / 255.0
)
if os.path.exists(
    f"{null_text_emb_path}/{input_img_name}_{inv_prompt_fg}_null_text.pt"
):
    null_text_emb = torch.load(
        f"{null_text_emb_path}/{input_img_name}_{inv_prompt_fg}_null_text.pt"
    ).to("cuda")
    init_noise = torch.load(
        f"{null_text_emb_path}/{input_img_name}_{inv_prompt_fg}_init_noise.pt"
    ).to("cuda")
else:
    null_text_emb, init_noise = diff_handles.invert_input_image(
        ten_img3, depth_fore, inv_prompt_fg
    )
    init_noise = init_noise[-1]
    torch.save(
        null_text_emb.detach().cpu(),
...
null_text_emb_fg, init_noise_fg, activations_fore, latent_image = (
    diff_handles.generate_input_image(
        depth=depth_fore,
        prompt=inv_prompt_fg,
        null_text_emb=null_text_emb,
        init_noise=init_noise,
    )
)

down_sampled_mask = torch.nn.functional.interpolate(
    torch.tensor(fg_mask[:, :, 0])
    .unsqueeze(0)
    .unsqueeze(0)
    .to(torch.float32)
    .to("cuda"),
...
with torch.no_grad():
    ...
    latent_image = (latent_image * 255).astype(np.uint8)
    cv2.imwrite(
        "./{}/{}_recon_fg.jpg".format(save_dir, input_img_name),
        cv2.cvtColor(latent_image, cv2.COLOR_RGB2BGR),
    )
...
for i in range(num_of_samples):
    ...
    cv2.imwrite(
        "./{}/{}_scene_comp_{}.png".format(save_dir, input_img_name, i),
        cv2.cvtColor(edited_img, cv2.COLOR_RGB2BGR),
    )
