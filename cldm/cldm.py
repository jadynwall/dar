...
class ControlledUnetModel(UNetModel):
    def forward(self, x, timesteps=None, context=None, control=None, only_mid_control=False, return_activations=False, **kwargs):
        hs = []
        ...
        for i, module in enumerate(self.output_blocks):
            ...
            # getting activations
            if(i in [5,8,11]):
                up_activations.append(h)

        h = h.type(x.dtype)
        # getting activations
        if return_activations:
            return self.out(h), up_activations
...
class ControlLDM(LatentDiffusion):
    ...
    def apply_model(self, x_noisy, t, cond, return_activations=False, *args, **kwargs):
        ...
        if cond['c_concat'] is None:
            if(not return_activations):
                ...
                eps, up_activations = diffusion_model(x=x_noisy, timesteps=t, context=cond_txt, control=None, 
                                                      only_mid_control=self.only_mid_control, return_activations=return_activations)
                return eps, up_activations
        else:
            ...
            if(not return_activations):
                ...
                eps, up_activations = diffusion_model(x=x_noisy, timesteps=t, context=cond_txt, control=control, 
                                                      only_mid_control=self.only_mid_control, return_activations=return_activations)
                return eps, up_activations
