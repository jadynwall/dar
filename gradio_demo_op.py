...
from run_inference_object_placement import (
    process_pairs,
    crop_back,
...
def analyze_depth_and_sam(background_image, reference_image, inv_prompt):
    ...
    # Process image pairs
    image_dict = process_pairs(
        ref_image, ref_mask, bg_np.copy(), tar_mask, shape_control=False
    )
    ...
    depth, sam_mask = get_depth_and_sam_mask(
        Image.fromarray(gt_image_cropped), is_relative_depth=True
    )
    ...
    plot_depth_bins(
        ...
        save_dir="./results/object_placement/gradio_infer",
        is_crop=True,
    )

    depth_top_path = f"./results/object_placement/gradio_infer/gradio_infer_depth_bins_front_crop.png"
    depth_front_path = (
        f"./results/object_placement/gradio_infer/gradio_infer_depth_bins_crop.png"
    )
...
def gradio_infer(
    ...
    if image_dict is None:
        image_dict = process_pairs(
            ...
            bg_np.copy(),
            tar_mask,
            shape_control=enable_shape_control,
        )
    ...
    ten_img3 = (
        torch.from_numpy(np.array(Image.fromarray(gt_image_cropped)))
        .float()
        .permute(2, 0, 1)
        .unsqueeze(0)
        .to("cuda")
    ...
    if (
        os.path.exists(f"{null_text_emb_path}/{image_name}_{inv_prompt}_null_text.pt")
        and not do_null_text_again
    ):
        null_text_emb = torch.load(
            f"{null_text_emb_path}/{image_name}_{inv_prompt}_null_text.pt"
        ).to("cuda")
        ddim_latents = torch.load(
            f"{null_text_emb_path}/{image_name}_{inv_prompt}_init_noise.pt"
        ).to("cuda")
        ...
        torch.save(
            null_text_emb.detach().cpu(),
            f"{null_text_emb_path}/{image_name}_{inv_prompt}_null_text.pt",
        )
        ddim_latent = [
            latent.detach().cpu().numpy().tolist() for latent in ddim_latents
        ]
        torch.save(
            torch.tensor(ddim_latent),
            f"{null_text_emb_path}/{image_name}_{inv_prompt}_init_noise.pt",
        )

    # Generate input image
    null_text_emb_fg, init_noise_fg, activations_fore, latent_image = (
        diff_handles.generate_input_image(
            depth=depth_fore,
            prompt=inv_prompt,
    ...
    with torch.no_grad():
        ...
        cv2.imwrite(
            f"{current_save_dir}/reconstructed_image.jpg",
    ...
    for i in range(num_samples):
        ...
        gen_image_pil.save(
            "./results/object_placement/gradio_infer/gradio_generated_image.png"
        )
...
def create_demo():
    with gr.Blocks(
        title="MPI Object Placement Demo",
        css="""
        .gradio-container {
    ...
    """,
        ...
        gr.Markdown(
            "**Two-Step Process:** 1) Upload images and analyze depth. 2) Choose depth value and generate object placement."
        )
        gr.Markdown(
            "**Instructions:** 1) Upload background image and draw a mask for bbox. 2) Upload reference object (RGBA). 3) Click 'Analyze Depth' to see depth distribution. 4) Adjust depth value based on analysis. 5) Generate the result."
        )
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                generate_button = gr.Button(
                    "Generate Object Placement", variant="primary"
                )
        ...
        with gr.Row():
            ...
            with gr.Column():
                depth_front_plot = gr.Image(
                    label="Depth Distribution (Front View)", show_label=True, height=300
                )
        ...
        with gr.Row():
            ...
            output_image = gr.Image(
                label="Generated Image", show_label=True, height=512
            )
        ...
        status_text = gr.Textbox(
            label="Status", value="Ready to generate", interactive=False
        )
        ...
        **Depth Distribution (Front View)**: Shows the depth distribution from the front. Use this to understand object placement.
        ...
        def analyze_with_status(bg_img, ref_img, inv_prompt_val):
            try:
                ...
                status_text.update(
                    "Depth analysis completed! Adjust depth value and generate."
                )
                return result
        ...
        def generate_with_status(
            bg_img, ref_img, depth_val, img_dict, inv_prompt_val, guidance_val, num_samp
        ):
            try:
        ...
        def clear_all():
            return (
                ...
                5.0,
                1,
                "a photo of a sofa in a livingroom",
                None,
                None,
                None,
                None,
