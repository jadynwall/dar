...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if processor is None:
            # processor = (
            #     AttnProcessor2_0(save_attentions=self.save_attentions) if hasattr(F, "scaled_dot_product_attention") and self.scale_qk else AttnProcessor(save_attentions=self.save_attentions)
            # )
            processor = (
                AttnProcessor2_0(save_attentions=self.save_attentions) if hasattr(F, "scaled_dot_product_attention") and self.scale_qk else AttnProcessor()
            )
    ...
    def set_use_memory_efficient_attention_xformers(
        ...
        if use_memory_efficient_attention_xformers:
            ...
            if is_lora:
                ...
                # but only if it has the default `scale` argument. TODO remove scale_qk check when we move to torch 2.1
                processor = (
                    AttnProcessor2_0(save_attentions=self.save_attentions)
                    if hasattr(F, "scaled_dot_product_attention") and self.scale_qk
                    else AttnProcessor(save_attentions=self.save_attentions)
                )
...
class AttnProcessor2_0:
    ...
    def __init__(self, save_attentions: bool = False):
        if not hasattr(F, "scaled_dot_product_attention"):
    ...
    def __call__(
        ...
        if self.save_attentions:
            dummy_value = torch.eye(key.shape[-2]).to(value.device, value.dtype)
            attn_probs = F.scaled_dot_product_attention(
                query, key, dummy_value[None, None, :, :], attention_mask, dropout_p=0.0, is_causal=False
            )[0]
        else:
            attn_probs = None
