...
class GuidedStableDiffuser(GuidedDiffuser):
    ...
    def guided_mpi_inference(
        ...
        with torch.no_grad():
            ...
            optimization_weight_schedule = [
                (0, [2.5, 2.5, 2.5], [1.25, 1.25, 1.25]),
                (1, [2.0, 2.0, 2.0], [1.25, 1.25, 1.25]),
                (2, [2.0, 2.0, 2.0], [1.25, 1.25, 1.25]),
                (3, [1.5, 1.5, 1.5], [1.25, 1.25, 1.25]),
        ...
        for t_idx, t in enumerate(tqdm(timesteps)):
            ...
            while iteration < self.conf.num_optsteps and t_idx < self.conf.guidance_max_step:
                ...
                fgw, bgw = guidance_weight_schedule(t_idx, iteration)
                bgw = fgw
                print("fg weights, bg weights : ", fgw, bgw)
    ...
    def guided_mpi_scene_comp(
        ...
        with torch.no_grad():
            ...
            for t_idx in range(self.conf.guidance_max_step):
                ...
                bg_weights = [0.0, 0.2, 8.5]
                # fg_weights = [0.0, 0.4, 8.5]
                # bg_weights = [0.0, 0.4, 1.5] # seems good some noise
                # fg_weights = [0.0, 0.4, 7.5]
    ...
    def guided_mpi_nvs_inference(
        ...
        if(len(activations_orig) == 1):
            activations_fore = activations_orig[0]
