...
def colorise_depth(depth_map):
    ...
    depth_map = ((depth_map - min_depth) / (max_depth - min_depth) * 255).astype(
        np.uint8
    )
...
def process_scene_composition(
    ...
    if background_image is None or foreground_image is None or mask_image is None:
        raise gr.Error(
            "Please upload all required images: background, foreground, and mask."
        )
    ...
    depth_control = depth_foreground * mask[:, :, 0] + depth_background * (
        1 - mask[:, :, 0]
    )
    depth_control = (
        torch.from_numpy(depth_control)
        .unsqueeze(0)
        .unsqueeze(0)
        .to(torch.float32)
        .to(device)
    ...
    depth_background_tensor = (
        torch.tensor(depth_background).unsqueeze(0).unsqueeze(0).to(device)
    )
    depth_foreground_tensor = (
        torch.tensor(depth_foreground).unsqueeze(0).unsqueeze(0).to(device)
    )

    background_tensor = (
        torch.tensor(np.array(background_image))
        .permute(2, 0, 1)
        .unsqueeze(0)
        .to(device)
        / 255.0
    ...
    foreground_tensor = (
        torch.tensor(np.array(foreground_image))
        .permute(2, 0, 1)
        .unsqueeze(0)
        .to(device)
        / 255.0
    ...
    try:
        ...
        bg_name = "background"
        if os.path.exists(
            f"{null_text_emb_path}/{bg_name}_{background_prompt}_null_text.pt"
        ):
            null_text_emb = torch.load(
                f"{null_text_emb_path}/{bg_name}_{background_prompt}_null_text.pt"
            ).to(device)
            init_noise = torch.load(
                f"{null_text_emb_path}/{bg_name}_{background_prompt}_init_noise.pt"
            ).to(device)
        else:
            null_text_emb, init_noise = diff_handles.invert_input_image(
                background_tensor, depth_background_tensor, background_prompt
            )
            ...
            torch.save(
                init_noise.detach().cpu(),
                f"{null_text_emb_path}/{bg_name}_{background_prompt}_init_noise.pt",
            )

        null_text_emb_bg, init_noise_bg, activations_back, _ = (
            diff_handles.generate_input_image(
                depth=depth_background_tensor,
        ...
        fg_name = "foreground"
        if os.path.exists(
            f"{null_text_emb_path}/{fg_name}_{foreground_prompt}_null_text.pt"
        ):
            null_text_emb = torch.load(
                f"{null_text_emb_path}/{fg_name}_{foreground_prompt}_null_text.pt"
            ).to(device)
            init_noise = torch.load(
                f"{null_text_emb_path}/{fg_name}_{foreground_prompt}_init_noise.pt"
            ).to(device)
        else:
            null_text_emb, init_noise = diff_handles.invert_input_image(
                foreground_tensor, depth_foreground_tensor, foreground_prompt
            )
            init_noise = init_noise[-1]
            torch.save(
                null_text_emb.detach().cpu(),
        ...
        null_text_emb_fg, init_noise_fg, activations_fore, _ = (
            diff_handles.generate_input_image(
                depth=depth_foreground_tensor,
...
def create_demo():
    ...
    """,
        ...
        gr.Markdown(
            "Upload a background image, foreground image, and mask, then provide text prompts to generate composed scenes."
        )
        gr.Markdown(
            "**Instructions:** Upload three images - background, foreground object, and a mask (white for foreground, black for background). Then provide text prompts describing each component and the desired final composition."
        )
        ...
        with gr.Row():
            with gr.Column():
                ...
                background_image = gr.Image(
                    label="Background Image", type="pil", height=256
                )
                foreground_image = gr.Image(
                    label="Foreground Image", type="pil", height=256
                )
                mask_image = gr.Image(
