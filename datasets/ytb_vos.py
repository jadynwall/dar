...
class YoutubeVOSDataset(BaseDataset):
    ...
    def get_sample(self, idx):
        ...
        end_frame_index = start_frame_index + np.random.randint(
            min_interval, len(frames) - start_frame_index
        )
        ...
        ref_image_path = (
            os.path.join(self.image_root, video_id, ref_image_name) + ".jpg"
        )
        tar_image_path = (
            os.path.join(self.image_root, video_id, tar_image_name) + ".jpg"
        )
        ref_mask_path = ref_image_path.replace("JPEGImages", "Annotations").replace(
            ".jpg", ".png"
        )
        tar_mask_path = tar_image_path.replace("JPEGImages", "Annotations").replace(
            ".jpg", ".png"
        )
