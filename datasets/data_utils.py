...
def new_box2squre(image, box):
    H, W = image.shape[0], image.shape[1]
    y1, y2, x1, x2 = box
    cx = (x1 + x2) // 2
    cy = (y1 + y2) // 2
    h, w = y2 - y1, x2 - x1
    # min_grow_h = min(cy - 0 - h//2, H - (cy + h//2))
    # min_grow_w = min(cx - 0 - w//2, W - (cx + w//2))
    # max_growable_len = min(min_grow_h, min_grow_w)
    # if(h + 2 * max_growable_len > 512 and w + 2 * max_growable_len > 512):
    #     if(min_grow_h > min_grow_w):
    #         h_new = min(512, w + 2 * max_growable_len)
    #         w_new = min(512, w + 2 * max_growable_len)
    #     else:
    #         h_new = min(512, h + 2 * max_growable_len)
    #         w_new = min(512, h + 2 * max_growable_len)
    # else:
    #     h_new = 512
    #     w_new = 512
    ...
    # y1 = cy - h_new // 2
    # y2 = cy + h_new // 2
    y1_ori, y2_ori, x1_ori, x2_ori = y1, y2, x1, x2
    w_new = max(h, w)
    h_new = max(h, w)
    # w_new, h_new = 512,512
    if(w_new < 0.7*512):
        w_new = min(max(w_new, 512), W, H)
    else:
        w_new = min(max(w_new, 768), W, H)
    h_new = w_new
    y_extra = h_new - h
    x_extra = w_new - w
    if(H < W):
        if(y1 > y_extra // 2 and y2 > H - y_extra // 2):
            y2 = H
            y1 = H - h_new
        elif(y1 < y_extra // 2 and y2 < H - y_extra // 2):
            y1 = 0
            y2 = h_new
        else:
            y1 = cy - h_new // 2
            y2 = cy + h_new // 2
        if(x1 > x_extra // 2 and x2 > W - x_extra // 2):
            x2 = W
            x1 = W - w_new
        elif(x1 < x_extra // 2 and x2 < W - x_extra // 2):
            x1 = 0
            x2 = w_new
        else:
            x1 = cx - w_new // 2
